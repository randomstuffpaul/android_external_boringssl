...
static void ecp_nistz256_windowed_mul(const EC_GROUP *group, P256_POINT *r,
  ...
  for (i = 0; i < num; i++) {
    ...
    if (!ecp_nistz256_bignum_to_field_elem(row[1 - 1].X, &point[i]->X) ||
      ...
      OPENSSL_PUT_ERROR(EC, ecp_nistz256_windowed_mul, EC_R_COORDINATES_OUT_OF_RANGE);
      goto err;
    }
...
static int ecp_nistz256_points_mul(
  ...
  if (scalar) {
    ...
    if (generator == NULL) {
      OPENSSL_PUT_ERROR(EC, ecp_nistz256_points_mul, EC_R_UNDEFINED_GENERATOR);
      goto err;
    }
    ...
    if (ecp_nistz256_is_affine_G(generator)) {
      if (BN_num_bits(scalar) > 256 || BN_is_negative(scalar)) {
        ...
        if (!BN_nnmod(tmp_scalar, scalar, &group->order, ctx)) {
          OPENSSL_PUT_ERROR(EC, ecp_nistz256_points_mul, ERR_R_BN_LIB);
          goto err;
        }
        scalar = tmp_scalar;
      }
...
static int ecp_nistz256_get_affine(const EC_GROUP *group, const EC_POINT *point,
  ...
  if (EC_POINT_is_at_infinity(group, point)) {
    OPENSSL_PUT_ERROR(EC, ecp_nistz256_get_affine, EC_R_POINT_AT_INFINITY);
    return 0;
  }
  ...
  if (!ecp_nistz256_bignum_to_field_elem(point_x, &point->X) ||
    ...
    OPENSSL_PUT_ERROR(EC, ecp_nistz256_get_affine, EC_R_COORDINATES_OUT_OF_RANGE);
    return 0;
  }
