...
int ec_GFp_nistp224_group_set_curve(EC_GROUP *group, const BIGNUM *p,
  ...
  if (BN_cmp(curve_p, p) ||
      BN_cmp(curve_a, a) ||
      BN_cmp(curve_b, b)) {
    OPENSSL_PUT_ERROR(EC, ec_GFp_nistp224_group_set_curve, EC_R_WRONG_CURVE_PARAMETERS);
    goto err;
  }
...
int ec_GFp_nistp224_point_get_affine_coordinates(const EC_GROUP *group,
  ...
  if (EC_POINT_is_at_infinity(group, point)) {
    OPENSSL_PUT_ERROR(EC, ec_GFp_nistp224_point_get_affine_coordinates, EC_R_POINT_AT_INFINITY);
    return 0;
  }
  ...
  if (x != NULL && !felem_to_BN(x, x_out)) {
    OPENSSL_PUT_ERROR(EC, ec_GFp_nistp224_point_get_affine_coordinates, ERR_R_BN_LIB);
    return 0;
  }
...
int ec_GFp_nistp224_points_mul(const EC_GROUP *group, EC_POINT *r,
  ...
  if (scalar != NULL) {
    ...
    if (!felem_to_BN(x, g_pre_comp[0][1][0]) ||
      ...
      OPENSSL_PUT_ERROR(EC, ec_GFp_nistp224_points_mul, ERR_R_BN_LIB);
      goto err;
    }
  ...
  if (num_points > 0) {
    ...
    if (secrets == NULL ||
      ...
      OPENSSL_PUT_ERROR(EC, ec_GFp_nistp224_points_mul, ERR_R_MALLOC_FAILURE);
      goto err;
    }
